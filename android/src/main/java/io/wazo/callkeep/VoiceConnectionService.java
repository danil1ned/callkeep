...
public class VoiceConnectionService extends ConnectionService {
  ...
  public static void setPhoneAccountHandle(
  ...
  public static void setAvailable(Boolean value) {
    Log.d(
      TAG,
      "[VoiceConnectionService] setAvailable: " + (value ? "true" : "false")
    );
    if (value) {
      setInitialized(true);
    }
  ...
  public static void setCanMakeMultipleCalls(Boolean value) {
    Log.d(
      TAG,
      "[VoiceConnectionService] setCanMakeMultipleCalls: " +
      (value ? "true" : "false")
    );
  ...
  public static void setInitialized(boolean value) {
    Log.d(
      TAG,
      "[VoiceConnectionService] setInitialized: " + (value ? "true" : "false")
    );
  ...
  public static void setState(String uuid, int state) {
    ...
    if (conn == null) {
      Log.w(
        TAG,
        "[VoiceConnectionService] setState ignored because no connection found, uuid: " +
        uuid
      );
      return;
    }
  ...
  public Connection onCreateIncomingConnection(
    ...
    Boolean isForeground = VoiceConnectionService.isRunning(
    ...
    Log.d(
      TAG,
      "[VoiceConnectionService] onCreateIncomingConnection, name:" +
      name +
      ", number" +
      number +
      ", isForeground: " +
  ...
  public Connection onCreateOutgoingConnection(
    ...
    Log.d(
      TAG,
      "[VoiceConnectionService] onCreateOutgoingConnection, uuid:" + callUUID
    );
  ...
  private Connection makeOutgoingCall(
    ConnectionRequest request,
    String uuid,
    Boolean forceWakeUp
  ) {
    ...
    Log.d(
      TAG,
      "[VoiceConnectionService] makeOutgoingCall, uuid:" +
      uuid +
      ", number: " +
      number +
      ", displayName:" +
    ...
    if (!isForeground || forceWakeUp) {
      Log.d(
        TAG,
        "[VoiceConnectionService] onCreateOutgoingConnection: Waking up application"
      );
      this.wakeUpApplication(uuid, number, displayName);
    } else if (!this.canMakeOutgoingCall() && isReachable) {
      Log.d(
        TAG,
        "[VoiceConnectionService] onCreateOutgoingConnection: not available"
      );
    ...
    if (!canMakeMultipleCalls) {
      Log.d(
        TAG,
        "[VoiceConnectionService] onCreateOutgoingConnection: disabling multi calls"
      );
    ...
    if (!Build.MANUFACTURER.equalsIgnoreCase("Samsung")) {
      Log.d(
        TAG,
        "[VoiceConnectionService] onCreateOutgoingConnection: initializing connection on non-Samsung device"
      );
  ...
  private void stopForegroundService() {
    ...
    try {
      ...
      Log.w(
        TAG,
        "[VoiceConnectionService] can't stop foreground service :" +
        e.toString()
      );
    }
  }
  ...
  private boolean isForegroundServiceConfigured() {
    ...
    try {
      return (
        foregroundSettings != null && foregroundSettings.hasKey("channelId")
      );
    } catch (Exception e) {
      // Fix ArrayIndexOutOfBoundsException thrown by ReadableNativeMap.hasKey
      Log.w(
        TAG,
        "[VoiceConnectionService] Not creating foregroundService due to configuration retrieval error" +
        e.toString()
      );
      return false;
    }
  ...
  private void wakeUpApplication(
    ...
    Log.d(
      TAG,
      "[VoiceConnectionService] wakeUpApplication, uuid:" +
      uuid +
      ", number :" +
      number +
      ", displayName:" +
    ...
    try {
      ...
      if (name != null) {
        Log.d(
          TAG,
          "[VoiceConnectionService] wakeUpApplication, acquiring lock for application:" +
          name
        );
      ...
      Log.w(
        TAG,
        "[VoiceConnectionService] wakeUpApplication, error" + e.toString()
      );
    }
  }
  ...
  private void wakeUpAfterReachabilityTimeout(ConnectionRequest request) {
    ...
    Log.d(
      TAG,
      "[VoiceConnectionService] checkReachability timeout, force wakeup, number :" +
      number +
      ", displayName: " +
      displayName
    );
  ...
  private void checkReachability() {
    ...
    new android.os.Handler()
      .postDelayed(
        new Runnable() {
          public void run() {
            instance.wakeUpAfterReachabilityTimeout(
              instance.currentConnectionRequest
            );
          }
        },
        2000
  ...
  private Connection createConnection(ConnectionRequest request) {
    ...
    Log.d(
      TAG,
      "[VoiceConnectionService] createConnection, callerNumber:" + callerNumber
    );
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      ...
      if (
        (
          phoneAccount.getCapabilities() & PhoneAccount.CAPABILITY_SELF_MANAGED
        ) ==
    ...
    List<Connection> conferenceConnections = new ArrayList<Connection>(
      otherConnections.values()
    );
  ...
  public void onCreateIncomingConnectionFailed(
    ...
    Log.w(
      TAG,
      "[VoiceConnectionService] onCreateIncomingConnectionFailed: " + request
    );
      ...
      .post(
        new Runnable() {
          ...
          public void run() {
            ...
            Log.d(
              TAG,
              "[VoiceConnectionService] startObserving, event count: " + count
            );

            for (Bundle event : delayedEvents) {
              String action = event.getString("action");
              HashMap attributeMap = (HashMap) event.getSerializable(
                "attributeMap"
              );

              currentConnectionService.sendCallRequestToActivity(
                action,
                attributeMap,
                false
              );
  ...
  private void sendCallRequestToActivity(
    ...
    Log.d(
      TAG,
      "[VoiceConnectionService] sendCallRequestToActivity, action:" + action
    );
  ...
  public static boolean isRunning(Context context) {
    ...
    List<RunningTaskInfo> tasks = activityManager.getRunningTasks(
      Integer.MAX_VALUE
    );
  ...
  private void checkForAppReachability(
